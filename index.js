...
const payTax = (income) => {
  const tax = 0.23;
  const fee = () => income * tax;
  return fee;
};

const payMyTax = payTax(8500);
console.log("--------------------------------------");
console.log("payTax:", payMyTax());

// console.log("Closures", closures("Kewin"));
...
console.log(
  "Common:",
  common(["b", 3, 4, 76, "c"], ["a", "b", 4, 76, 21, "e"])
);
...
console.log(
  "Different:",
  different(["b", 3, 4, 76, "c"], ["a", "b", 4, 76, 21, "e"])
);
...
const tuples = (arr1, arr2) => arr1.map((item, index) => [item, arr2[index]]);
